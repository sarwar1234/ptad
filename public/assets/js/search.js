...
function renderNormalResult(r) {
    ...
    const cardClass = topNote && topNote.priority === 1 ? 'is-blocked' : (topNote && topNote.priority <= 2 ? 'is-caution' : '');
        ...
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
            <div>
                ...
                <div class="text-muted small mt-1">
                    ...
                    ${r.import_country ? ' · Market: ' + flagImg(r.import_country_iso2) + ' ' + escapeHtml(r.import_country) : ''}
                    ${r.member_country ? ' · Member: ' + flagImg(r.member_country_iso2) + ' ' + escapeHtml(r.member_country) : ''}
                </div>
...
async function runSearch(query, mode, offset) {
    ...
    if (query === '') {
        container.innerHTML = '<p class="text-muted">Enter a search term above.</p>';
        meta.textContent = '';
        return;
    }

    container.innerHTML = '<div class="ptad-skeleton mb-3" style="height:90px;"></div>'.repeat(3);

    const endpoint = mode === 'description' ? '/api/search-description' : '/api/search';
    ...
    try {
        ...
        if (data.data.length === 0) {
            container.innerHTML = '<p class="text-muted">No results found. Try a shorter code or a different term.</p>';
            meta.textContent = '';
            return;
        }
        ...
        container.innerHTML = data.data.map(r =>
            r.agreement_code === 'SAFTA' ? renderSaftaResult(r) : renderNormalResult(r)
        ).join('');
...
function renderPagination(meta) {
    ...
    let html = '<div class="d-flex align-items-center gap-1 flex-wrap justify-content-center">';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (initialQuery) {
        ...
        document.getElementById('resultsContainer').innerHTML = '<p class="text-muted">Enter a search term above.</p>';
    }
});
